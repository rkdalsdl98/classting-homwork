...
export const NESTIA_CONFIG: SDK.INestiaConfig = {
  input: ["src/modules/*/controller"], // 다수에 경우 ["src/controllers"] 같이 컨트롤러가 모여있는곳을 지정 해주기도 함
  ...
  swagger: {
    info: { 
      ...
      title: "ClassTing Homework API",
      description: "Private으로 지정 된 API는 토큰 검증을 통해 사용되어야 하는 API이며, 해당 API에서는 검증을 구현 하지 않아 프로퍼티로 받아 사용합니다.",
      version: "1.0.0",
    },
